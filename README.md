...
The TUI transcript also has package-owned renderers for `subagent` and `subagent_supervisor` calls and results. Calls show complete action-specific input, including full task, continuation, guidance, reply text, IDs, and cwd. Nonterminal immediate results use compact one-line acknowledgements; an already-terminal or failed result may append its complete final output/error. Retained-run lists and lifecycle notifications show only status-relevant information: complete waiting requests, complete terminal output/error, and explicitly labeled live response/tool activity for active runs. Terminal entries never repeat stale live response text. Rendering is independent of tool expansion state, display-only, and neither creates a duplicate message nor copies renderer details into model context.
