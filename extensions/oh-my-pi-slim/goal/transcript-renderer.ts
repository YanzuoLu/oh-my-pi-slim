...
import { goalStatusGlyph, sanitizeGoalBody, sanitizeGoalText } from "./widget.js";
...
function validStatus(value: unknown): value is GoalStatus {
  return value === "active" || value === "retry_wait" || value === "paused" || value === "completed";
}
...
function goalFromValue(value: unknown): PublicGoalState | undefined {
  ...
  if (!goal || !validStatus(goal.status) || typeof goal.abstract !== "string" || typeof goal.objective !== "string" ||
      ...
      !(goal.evidence === null || Array.isArray(goal.evidence) && goal.evidence.every((item) => typeof item === "string"))) return;
  return {
    status: goal.status,
 ...
 * that already had none, and every ordinary empty `check`, stay hollow and dim.
 */
...
function resultSummary(action: GoalResultAction, goal: PublicGoalState | null, changed: boolean, theme: Theme): Text {
  ...
  if (action === "pause") suffix = changed ? "paused" : "already paused · no change";
  ...
  const glyph = changed || action === "check" ? goalStatusGlyph(goal.status, theme) : theme.fg("dim", "○");
  return new Text(
...
export function renderGoalResult(
  ...
  const action = (asString(args.action) ?? "check") as GoalResultAction;
