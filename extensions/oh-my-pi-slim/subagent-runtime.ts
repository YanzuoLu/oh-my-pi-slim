...
export class OmpsSubagentRuntime {
  ...
  registerTools(): void {
    this.pi.registerTool({
      name: "subagent",
      label: "Subagent",
      description: "Start a package specialist asynchronously and receive a new run ID with status starting. Follow-up notifications deliver waiting requests and complete terminal results. Use list for retained runs, steer for running work, interrupt for active work, and resume for saved child sessions.",
      ...
      promptGuidelines: [
        ...
        "A waiting subagent follow-up notification contains the run ID, request ID, reason, and message.",
        "A completed, failed, or interrupted subagent follow-up notification contains the run ID and every stored output and error field for that transition.",
        "subagent list returns retained runs with their current statuses and stored results.",
        "subagent steer sends a message to a running run.",
        "subagent interrupt sends an interruption request to a starting, running, or waiting run; its follow-up notification reports the actual terminal status.",
    ...
    this.pi.registerTool({
      ...
      description: "View waiting specialist requests and reply to continue a specialist.",
      ...
      promptGuidelines: [
        ...
        "After subagent_supervisor reply, the same run ID returns to running with saved child-session context; its next waiting, completed, failed, or interrupted transition sends a follow-up notification.",
      ],
  ...
  private sendNotification(run: PersistedRun, event: RunStatus): void {
  ...
  private deliverPendingNotification(id: string): void {
    ...
    if (!run?.notificationPending) return;
    const pending = run.notificationPending;
    try { this.sendNotification(run, pending); } catch { return; }
    const latest = this.registry.get(id);
    if (latest?.notificationPending === pending) this.updateRun(id, { notificationPending: undefined });
  }
  ...
  private async executeSubagent(input: RuntimeInput) {
    ...
    if (action === "list") {
      ...
      const runs = this.registry.list().map((run) => this.formatRun(run));
