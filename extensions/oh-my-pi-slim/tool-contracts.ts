...
export const SUBAGENT_TOOL_DESCRIPTIONS = {
  tool: markdown(
    ...
    "",
    "`fork: true` is the default because most delegation continues shared work. Use `fork: false` when supervisor reasoning would invalidate the child's answer. That covers independent review, independent reproduction, blind comparison, genuinely diverse alternatives, and fresh-user validation. A forked reviewer confirms the supervisor's conclusion instead of testing it. This gate is about correctness and never about saving context. A self-contained `message` is required either way. Writer lanes almost always fork. A stale file snapshot is not a reason to drop the fork. Tell the child to re-read the current state instead.",
    "",
    "`fork` and `resume` carry different context. A forked child receives the supervisor's conversation. A resumed run continues the source child's own session. That session holds the files it read, the searches it ran, and its partial work. The supervisor never held that child-side context. Choose by where the needed context lives. Use `resume` when the continuation depends on the child's own working state. Use `create` with `fork` when it depends on supervisor-side information. That includes a new user requirement, another lane's output, or a result already returned.",
    "",
    "## Actions",
    "",
    ...
    "- `resume` starts a new run that continues a terminal run's own child session, reusing that child's working context. It returns new and source IDs plus status.",
