...
import { SUBAGENT_WIDGET_GLYPHS } from "./subagent-widget-glyphs.js";
...
export function formatWidgetSessionTokens(
  ...
  if (compactions > 0) annotations.push(theme.fg("dim", `${SUBAGENT_WIDGET_GLYPHS.compactions}${compactions}`));
...
export function formatWidgetTurns(turnCount: number): string {
  return `${SUBAGENT_WIDGET_GLYPHS.turns}${turnCount}`;
}
