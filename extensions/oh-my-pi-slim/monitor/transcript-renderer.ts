...
function operationalStateFromValue(value: unknown): MonitorOperationalState | undefined {
  ...
  if (!id || abstract === undefined || command === undefined || cwd === undefined || pid === undefined || !status || !createdAt ||
      !updatedAt || lastOutputAt === undefined || endedAt === undefined || exitCode === undefined || signal === undefined ||
      error === undefined || notificationCount === undefined || !logPath || logBytes === undefined ||
      logLines === undefined || droppedBytes === undefined || droppedLines === undefined ||
  ...
  return {
    ...
    notificationCount, logPath, logBytes, logLines, droppedBytes, droppedLines,
    returned, omitted, truncated, combined,
  };
}
...
export function renderMonitorCall(argsValue: unknown, theme: Theme, context: ToolRenderContextLike = {}): Component {
  ...
  } else if (action === "stop" || action === "check") {
...
function updateNotification(value: UnknownRecord): UpdateNotification | undefined {
  ...
  if (!id || abstract === undefined || !status || exitCode === undefined || signal === undefined ||
      error === undefined || !lines || omitted === undefined || truncated === undefined) return;
  return { id, abstract, status, exitCode, signal, error, lines, omitted, truncated };
}

/** One layout for event and terminal updates. */
function renderUpdateNotification(details: UpdateNotification, expanded: boolean, theme: Theme): Component {
  ...
  const tail = theme.fg("customMessageText", ` · ${details.status}`);
