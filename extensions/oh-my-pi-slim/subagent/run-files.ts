...
/**
 * Removes one child session file only when it truly resides inside this supervisor session's child session root.
...
export function createForkSessionFile(
  ...
  const id = randomUUID();
  const filePath = join(sessionDir, `${timestamp.replace(/[:.]/g, "-")}_${id}.jsonl`);
  const header = {
    type: "session",
    version,
    id,
  ...
  try {
    ...
    try { rmSync(filePath, { force: true }); } catch { /* preserve the original write error */ }
    throw error;
  }
  return filePath;
}
