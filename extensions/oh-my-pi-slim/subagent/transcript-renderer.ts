...
function runIdentity(run: UnknownRecord): { id: string; status: string } {
  return {
...
function compactRunHeaderParts(
  ...
  return {
    head: `${formatSemanticGlyphPrefix(statusGlyph(status, theme))}${theme.fg("toolTitle", theme.bold("Subagent"))} ${theme.fg("accent", `[${identity.id}]`)}`,
...
function immediateAck(
  ...
  const { id, status } = runIdentity(run);
  const identity = `Subagent [${id}]`;
  ...
  } else if (terminal && action === "steer") {
    text = `${identity} · already ${status}`;
  } else if (action === "resume") {
    const source = asString(args.id) ?? asString(run.sourceRunId) ?? "unknown source";
    text = `Resumed [${source}] → ${identity} · ${status}`;
  } else if (action === "reply") {
    text = `Replied · ${identity} · ${status}`;
  } else if (action === "steer") {
    text = `Steer requested · ${identity} · ${status}`;
  } else if (action === "interrupt") {
    glyph = theme.fg("warning", "!");
    text = `Interrupt requested · ${identity} · ${status}`;
  } else {
    text = `Started ${identity} · ${status}`;
  }
  const container = new Container();
  container.addChild(new Text(`${formatSemanticGlyphPrefix(glyph)}${theme.fg("toolOutput", text)}`, 0, 0));
  const ownsFinalResult = action !== "steer";
...
function renderRunCheck(run: UnknownRecord, theme: Theme, expanded: boolean): Container {
  const container = new Container();
...
function renderRunList(runs: unknown[], theme: Theme, expanded: boolean): Container {
  ...
  const label = active > 0 ? theme.bold(`Subagents (${terminal}/${runs.length})`) : `Subagents (${terminal}/${runs.length})`;
  const container = new Container();
  container.addChild(new Text(
...
export function renderSubagentCall(argsValue: unknown, theme: Theme, context: ToolRenderContextLike = {}): Component {
  ...
  const container = new Container();
  const collapsedCheckId = action === "check" && !expanded && typeof args.id === "string"
    ? ` · ${safeFirstLine(args.id)}`
    : "";
  container.addChild(styledTitle(
    theme,
    "subagent",
    `· ${action}${collapsedCheckId}${expanded ? "" : " (ctrl+o to expand)"}`,
  ));

  if (action === "create") {
    addField(container, theme, "Abstract", args.abstract, "(pending)");
    if (expanded) {
      addField(container, theme, "Cwd", args.cwd ?? context.cwd, "(supervisor session cwd)");
      addField(container, theme, "Fork", args.fork ?? true);
      if (typeof args.message === "string") addFullSection(container, theme, "Message", args.message);
      else addField(container, theme, "Message", undefined, "(pending)");
    }
  } else if (action === "resume") {
  ...
  } else if (action === "check" || action === "interrupt" || action === "delete") {
    if (action !== "check" || expanded) addField(container, theme, "Run", args.id, "(pending)");
  } else if (action === "reply") {
    addField(container, theme, "Run", args.id, "(pending)");
    if (expanded) {
      if (typeof args.message === "string") addFullSection(container, theme, "Message", args.message);
...
export function renderSubagentResult(
  ...
  if (action === "check" && run) return spacedToolResult(renderRunCheck(run, theme, expanded));
