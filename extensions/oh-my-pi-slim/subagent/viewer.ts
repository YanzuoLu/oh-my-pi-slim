...
import type { RunStatus } from "./core.js";
import { formatSubagentModel } from "./model-display.js";
import { formatWidgetTokens, formatWidgetTurns } from "./widget-display.js";
...
export class SubagentViewerComponent implements Component, Focusable {
  ...
  private statusLines(model: ViewerModel, width: number): string[] {
    ...
    const title = [
      ...
      theme.fg("dim", `[${sanitizeViewerInline(run.id)}]`),
...
export class SubagentViewer implements SubagentViewerKeyTarget {
  ...
  private refresh(generation: number): void {
    ...
    this.adoptSnapshot(snapshot);
    // Membership is the retained set and snapshot order is creation-stable, so lifecycle and
    // `updatedAt` changes keep the selected ID in place. An ID leaves only after `subagent delete`
    // or `subagent clear`, which is exactly when the neighbour rule has to run.
