...
export type GoalStatus = "active" | "retry_wait" | "paused" | "completed" | "cancelled";
...
export const goalParameters = Type.Object({
  ...
  objective: Type.Optional(Type.String({
    description: "For create or modify, provide the complete Goal objective.",
  })),
  ...
  reason: Type.Optional(Type.String({
    description: "For pause or cancel, provide the reason.",
  })),
...
function trimmedStringArray(value: unknown, field: string, minimum = 1, maximum = 8): string[] {
  if (!Array.isArray(value) || value.length < minimum || value.length > maximum) {
    throw new Error(`${field} must contain from ${minimum} through ${maximum} non-empty strings.`);
  }
...
export function parseGoalSnapshot(value: unknown): GoalSnapshot | undefined {
  ...
  if (!["active", "retry_wait", "paused", "completed", "cancelled"].includes(String(goal.status))) return;
  ...
  if (!nullableString(goal.pauseReason) || !nullableIso(goal.nextRetryAt) || !nullableString(goal.lastProviderError) ||
      !nullableString(goal.cancelReason) || !nullableStringArray(goal.evidence)) return;
  ...
  if ((status === "completed" || status === "cancelled") !== (goal.endedAt !== null)) return;
  if (status === "completed") {
    if (!Array.isArray(goal.evidence) || goal.evidence.length !== goal.criteria.length || goal.cancelReason !== null) return;
  } else if (goal.evidence !== null) return;
  if (status === "cancelled") {
    if (goal.cancelReason === null) return;
  } else if (goal.cancelReason !== null) return;
  if (status === "paused") {
    if (goal.pauseReason === null) return;
  } else if (goal.pauseReason !== null) return;
...
function goalContractFields(goal: Pick<PublicGoalState, "abstract" | "objective" | "criteria">): string[] {
  return [
...
export function goalActivationContent(kind: "created" | "modified" | "resumed", goal: PublicGoalState): string {
  const first = kind === "created"
    ? "Goal created and active."
    : kind === "modified" ? "Goal modified and active." : "Goal resumed and active.";
  return [
    first,
    "",
...
export function goalContinuationContent(goal: PublicGoalState): string {
  return [
...
export function goalPhaseReminder(abstract: string): string {
...
export class GoalRuntime {
  ...
  phaseReminder(): string | undefined {
    return this.isActive() && this.snapshot ? goalPhaseReminder(this.snapshot.goal.abstract) : undefined;
  }
  ...
  async execute(inputValue: GoalInput): Promise<ReturnType<typeof toolText>> {
    const input = record(inputValue, "goal input");
    const action = trimmedString(input.action, "action") as GoalAction;
    if (!GOAL_ACTIONS.includes(action)) throw new Error(`Unsupported Goal action: ${action}`);
  ...
  private create(input: Record<string, unknown>): ReturnType<typeof toolText> {
    if (this.snapshot && this.snapshot.goal.status !== "completed" && this.snapshot.goal.status !== "cancelled") {
      throw new Error(`create requires no Goal or a terminal Goal; the current Goal is ${this.snapshot.goal.status}.`);
    }
    const now = this.nowIso();
  ...
  private pause(input: Record<string, unknown>): ReturnType<typeof toolText> {
    ...
    if (snapshot.goal.status === "completed" || snapshot.goal.status === "cancelled") {
      throw new Error(`pause is invalid for terminal Goal status ${snapshot.goal.status}.`);
    }
    if (snapshot.goal.status === "paused") {
      return statusReceipt("Goal is already paused.", snapshot.goal, false);
    }
    const next = cloneSnapshot(snapshot);
    next.generation += 1;
    next.goal.status = "paused";
    next.goal.updatedAt = this.nowIso();
    next.goal.pauseReason = trimmedString(input.reason, "reason");
    next.goal.nextRetryAt = null;
    this.clearRetryTimer();
    this.store(next);
    return statusReceipt("Goal paused.", next.goal, true);
  }

  private resume(): ReturnType<typeof toolText> {
    const snapshot = this.snapshot as GoalSnapshot;
    if (snapshot.goal.status === "completed" || snapshot.goal.status === "cancelled") {
      throw new Error(`resume is invalid for terminal Goal status ${snapshot.goal.status}.`);
    }
    if (snapshot.goal.status === "active") {
      return statusReceipt("Goal is already active.", snapshot.goal, false);
    }
    const next = cloneSnapshot(snapshot);
    next.generation += 1;
  ...
  private complete(input: Record<string, unknown>): ReturnType<typeof toolText> {
    const snapshot = this.snapshot as GoalSnapshot;
    if (snapshot.goal.status !== "active") throw new Error(`complete requires an active Goal; the current Goal is ${snapshot.goal.status}.`);
    const evidence = trimmedStringArray(input.evidence, "evidence");
    if (evidence.length !== snapshot.goal.criteria.length) {
      throw new Error(`evidence must contain exactly ${snapshot.goal.criteria.length} items, one per completion criterion.`);
    }
    const now = this.nowIso();
    const next = cloneSnapshot(snapshot);
    next.generation += 1;
    ...
    this.clearRetryTimer();
    this.store(next);
    return statusReceipt("Goal completed.", next.goal, true);
  }

  private cancel(input: Record<string, unknown>): ReturnType<typeof toolText> {
    const snapshot = this.snapshot as GoalSnapshot;
    if (snapshot.goal.status === "completed" || snapshot.goal.status === "cancelled") {
      throw new Error(`cancel is invalid for terminal Goal status ${snapshot.goal.status}.`);
    }
    const now = this.nowIso();
    const next = cloneSnapshot(snapshot);
    next.generation += 1;
    ...
    next.goal.updatedAt = now;
    next.goal.endedAt = now;
    next.goal.cancelReason = trimmedString(input.reason, "reason");
    next.goal.pauseReason = null;
    next.goal.retryAttempt = 0;
    next.goal.nextRetryAt = null;
    next.goal.lastProviderError = null;
  ...
  private requireMutable(action: string): GoalSnapshot {
    const snapshot = this.snapshot as GoalSnapshot;
    if (snapshot.goal.status === "completed" || snapshot.goal.status === "cancelled") {
      throw new Error(`${action} is invalid for terminal Goal status ${snapshot.goal.status}.`);
    }
    return snapshot;
  }
  ...
  private enterRetryWait(error: string): void {
    ...
    next.goal.lastProviderError = typeof error === "string" && error.trim() ? error.trim() : "Provider request failed.";
    this.store(next);
  ...
  private sendStateMessage(message: { event: GoalAutomaticEvent; reason: string; goal: PublicGoalState }): void {
    this.pi.sendMessage({
      ...
      content: `Goal state changed: ${message.event}.\nReason: ${message.reason}\n${JSON.stringify(message.goal, null, 2)}`,
      display: true,
      details: {
